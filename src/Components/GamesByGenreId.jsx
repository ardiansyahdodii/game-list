...
const GamesByGenreId = ({ gameList, selectedGenreName }) => {
    ...
    return (
        <div className='mt-5'>
            <h2 className='text-[30px] font-bold dark:text-white'>{selectedGenreName} Games</h2>
            <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mt-5'>
            {gameList.map((item) => (
                <div key={item.id} className='bg-slate-400 p-3 rounded-lg pb-10 group hover:scale-110 transition-all ease-in-out duration-300 cursor-pointer'>
